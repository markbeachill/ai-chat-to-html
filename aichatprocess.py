...
# Matches $$...$$ (display) and $...$ (inline) math spans. Display first so the
# inline pattern doesn't eat the doubled dollars. We protect these from the
# Markdown processor, which would otherwise mangle backslashes (\frac -> \\frac)
# and underscores inside formulae.
_MATH_RE = re.compile(r'(\$\$.+?\$\$|\$[^$\n]+?\$)', re.DOTALL)


def _protect_math(text: str):
    """Replace math spans with placeholders before Markdown runs. Returns the
    modified text and a list of the original spans to restore afterwards."""
    spans = []
...
def _restore_math(html_fragment: str, spans) -> str:
    for i, span in enumerate(spans):
        html_fragment = html_fragment.replace(f"\x00MATH{i}\x00", span)
    return html_fragment


# Matches <img ... src="..."> emitted by Python-Markdown.
_IMG_RE = re.compile(r'<img\b[^>]*?\bsrc="([^"]*)"[^>]*>', re.IGNORECASE)


def _filter_images(html_fragment: str) -> str:
    """Keep only http(s) image URLs; replace local/relative ones with a visible
    placeholder. The tool never references local files: a pasted transcript may
    contain ![](screenshot.png) pointing at something we don't have and
    shouldn't try to load, and a relative src could leak a path. Remote images
    (![](https://...)) are passed through as-is."""
    def repl(m):
        src = m.group(1).strip()
        if src.lower().startswith(("http://", "https://")):
            # Add lazy loading + max-width so a remote image behaves in the page.
            tag = m.group(0)
            if "loading=" not in tag:
                tag = tag[:-1] + ' loading="lazy">'
            return tag
        # Local/relative/data: not supported — show a clear placeholder.
        return ('<span class="ai-chat-img-missing" '
                'style="font-style:italic;opacity:0.7;">[image not included]</span>')
    return _IMG_RE.sub(repl, html_fragment)


def _convert_body(md, body: str) -> str:
    """Convert one turn's Markdown to HTML, protecting math spans and filtering
    images. Shared by the chat and Word builders."""
    if not body.strip():
        return ""
    protected, spans = _protect_math(body)
    md.reset()
    out = md.convert(protected)
    out = _restore_math(out, spans)
    return _filter_images(out)


def turn_to_html(md: "markdown.Markdown", role: str, body: str) -> str:
    inner = _convert_body(md, body)
...
# Opt-in math rendering. KaTeX is loaded from a CDN and renders $...$ and
# $$...$$ in the browser. This is the one feature that makes the page depend on
# an external resource, so it is off unless --math is passed. It does not apply
# to the Word output (Word pastes static HTML and won't run the script).
def _katex_head(enabled: bool) -> str:
    if not enabled:
        return ""
    ...
    return (
        ...
        f'  <script defer src="{base}/contrib/auto-render.min.js"\n'
...
def build_html(turns, css_href: str, title: str, math: bool = False) -> str:
...
<head>
  ...
  <link rel="stylesheet" href="{html.escape(css_href, quote=True)}">{_katex_head(math)}
</head>
<body>
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--math", action="store_true",
                        help="Render LaTeX math written between $...$ or $$...$$ "
                             "using KaTeX (loaded from a CDN). Off by default so "
                             "the page stays self-contained. Chat page only; the "
                             "Word version shows math as plain text.")
    ...
    page = build_html(turns, css_href=css_href, title=args.title, math=args.math)
